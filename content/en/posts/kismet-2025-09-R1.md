...
* Add version and remote connection type to remote datasources so help identify
  remote connections
* Google Protobufs now optional, required only for the now-legacy V2 datasource
  ...
  protobufs libraries.
* Start deprecating the V2 datasource/IPC protocol; compatibility remains to
...
* Rewrite JSON name conversion to compile-time functor for speed
...
* Rework exec'd IPC descriptor and stderr/out mapping in datasource capture
  framework
* Fix IPC handling closed exec'd pipes in datasource capture framework
